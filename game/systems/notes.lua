...
function NotesSystem:close_notes(not_close)
	if not_close then if not (type(not_close) == "boolean") then error("Assertion failed: type(not_close) == \"boolean\"") end end
...
NotesSystem["on_list_cursor_update_" .. "notes"] = function(self, e_hovered)
	if not (self.pool_item:has(e_hovered)) then error("Assertion failed: self.pool_item:has(e_hovered)") end
