...
function Movement:update(dt)
	for _, e in ipairs(self.pool_walk) do
		...
		local diff = random_walk.orig_pos:distance(e.pos.value)
		local body = e.body
...
function Movement:update_speed_data(e, anim_name)
	if not (e.__isEntity and e.animation and e.speed and e.speed_data and e.body) then error("Assertion failed: e.__isEntity and e.animation and e.speed and e.speed_data and e.body") end
	if not (type(anim_name) == "string") then error("Assertion failed: type(anim_name) == \"string\"") end
