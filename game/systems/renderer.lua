...
for k in pairs(renderer_per_pool) do
	if not (Renderer.__definition[k]) then error(k .. " must exists") end
end
...
local function get_list(self, e_or_bool)
	if e_or_bool then if not (type(e_or_bool) == "boolean" or e_or_bool.__isEntity) then error("Assertion failed: type(e_or_bool) == \"boolean\" or e_or_bool.__isEntity") end end
...
function Renderer:pool_on_added(pool, e)
	...
	elseif pool == self.pool_text and e.sdf then
		if not (e.font_sdf) then error("sdf must have font_sdf") end
		if not (not e.font) then error("sdf must NOT have font") end
	elseif pool == self.pool_static_text then
		if not (not e.sdf) then error("static_font can NOT use sdf font") end
...
function Renderer:draw(is_ui)
	if is_ui then if not (type(is_ui) == "boolean") then error("Assertion failed: type(is_ui) == \"boolean\"") end end
