...
function List:init(world)
	...
	self.pool.onAdded = function(pool, e)
		...
		if not (group) then error(group_id .. " does not exist") end
...
function List:create_list_group(id, vertical_only, per_page, use_max_threshold, limit)
	if not (type(id) == "string") then error("Assertion failed: type(id) == \"string\"") end
	if not (type(vertical_only) == "boolean") then error("Assertion failed: type(vertical_only) == \"boolean\"") end
	if not (type(per_page) == "number" and per_page > 0) then error("Assertion failed: type(per_page) == \"number\" and per_page > 0") end
	if not (self.groups[id] == nil) then error(id .. " was already added") end
	if use_max_threshold then if not (type(use_max_threshold) == "boolean") then error("Assertion failed: type(use_max_threshold) == \"boolean\"") end end
	if limit then if not (use_max_threshold and type(limit) == "number" and limit > 0) then error("Assertion failed: use_max_threshold and type(limit) == \"number\" and limit > 0") end end
	self.groups[id] = {
		entities = {},
		cursor = 1,
		offset = 0,
...
function List:create_list_group_grid(id, rows, cols)
	if not (type(id) == "string") then error("Assertion failed: type(id) == \"string\"") end
	if not (self.groups[id] == nil) then error(id .. " was already added") end
	if not (type(rows) == "number" and rows > 0) then error("Assertion failed: type(rows) == \"number\" and rows > 0") end
	if not (type(cols) == "number" and cols > 0) then error("Assertion failed: type(cols) == \"number\" and cols > 0") end
	self.groups[id] = {
		entities = {},
		cursor = 1,
		offset = 0,
...
function List:update_list(group)
	...
	if Inputs.pressed("interact") then
		...
		if not (e) then error("Assertion failed: e") end
...
function List:update_list_grid(group)
	...
	if Inputs.pressed("interact") then
		local index = group.per_page * group.offset + group.cursor
		if not (group.entities[index]) then error("Assertion failed: group.entities[index]") end
		local signal = "on_list_item_interact_" .. self.focused
...
function List:update_cursor(index)
	if index then if not (type(index) == "number" and index > 0) then error("Assertion failed: type(index) == \"number\" and index > 0") end end
	local group = self.groups[self.focused]
	...
	if e_hovered then
		...
		local signal = "on_list_cursor_update_" .. self.focused
...
function List:set_focus_list(group_id, override_cursor)
	local group = self.groups[group_id]
	if group_id then if not (type(group_id) == "string" and group) then error("Assertion failed: type(group_id) == \"string\" and group") end end
