...
function Systems:set_system_to(id, bool)
	...
	if not (system_classes[id] ~= nil) then error("Assertion failed: system_classes[id] ~= nil") end
...
function Systems:inventory_to_notes(reversed)
	if reversed then if not (type(reversed) == "boolean") then error("Assertion failed: type(reversed) == \"boolean\"") end end
