...
function GameStatesSystem:switch_state(next_state, dur, delay)
	if not (type(next_state) == "string") then error("Assertion failed: type(next_state) == \"string\"") end
	if dur then if not (type(dur) == "number") then error("Assertion failed: type(dur) == \"number\"") end end
	...
	if GameStates.current_id == next_state then return end
