...
function DialoguesSystem:create_e_dialogue()
	...
	if not (camera) then error("camera resource should be set by now") end
...
function DialoguesSystem:spawn_dialogue_ex(dialogue_t, signal_after, ...)
	...
	if not (#dialogue_t ~= 0) then error("Assertion failed: #dialogue_t ~= 0") end
	if signal_after then if not (type(signal_after) == "string") then error("Assertion failed: type(signal_after) == \"string\"") end end
	...
	if e.has_choices then
		...
		if on_d_end then
			...
			e:remove("dialogue_meta")
				...
				:remove("item_id")
				:remove("has_choices")
			self.e_dialogue:remove("dialogue_meta")
				:remove("text_t")
				:remove("text")
				:remove("item_id")
				:remove("has_choices")
...
function DialoguesSystem:update_dialogues(new_dialogues_t)
	if not (type(new_dialogues_t) == "table") then error("Assertion failed: type(new_dialogues_t) == \"table\"") end
...
function DialoguesSystem:show_choices()
	...
	for _, str in ipairs(c.value) do
		...
		Concord.entity(self.world):assemble(Assemblages.UI.choice,
				id, str, choice_t, x, y)
