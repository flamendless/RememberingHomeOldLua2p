...
function PostProcessing:setup_post_process(t)
	if not (type(t) == "table" and #t > 0) then error("Assertion failed: type(t) == \"table\" and #t > 0") end
...
function PostProcessing:set_post_process_effect(id, bool)
	...
	if not (false) then error(id .. " not found in registered pp effects") end
end

function PostProcessing:ev_pp_invoke(id, str_fn, ...)
	if not (type(id) == "string") then error("Assertion failed: type(id) == \"string\"") end
	if not (type(str_fn) == "string") then error("Assertion failed: type(str_fn) == \"string\"") end
	for _, effect in ipairs(self.effects) do
		local t = effect.get_type and effect:get_type() or effect:type()
		if t == id then
	...
	if not (false) then error(id .. " was not registered") end
end
...
function PostProcessing:apply_post_process(canvas)
	...
	if not (canvas:type() == "CustomCanvas") then error("Assertion failed: canvas:type() == \"CustomCanvas\"") end
