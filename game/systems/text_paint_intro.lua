...
function TextPaintIntro:fade_text(e, dur, on_finish)
	...
	if not (type(on_finish) == "function") then error("Assertion failed: type(on_finish) == \"function\"") end
...
function TextPaintIntro:generate_paint(e, dur_in, dur_out)
	...
	if dur_out then if not (type(dur_out) == "number") then error("Assertion failed: type(dur_out) == \"number\"") end end
	...
	local paint = Concord.entity(self.world)
		...
		:give("transform", 0, vec2(sx, 0), vec2(0.5, 0.5))
		:give("color", {1, 1, 1, 0})
		:give("layer", "text", 1)
