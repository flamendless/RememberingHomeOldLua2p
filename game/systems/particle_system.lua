...
function ParticleSystem:set_particle_system(id_or_ids, mode, ...)
	...
	if not (is_str or is_tbl) then error("Assertion failed: is_str or is_tbl") end
	if not (type(mode) == "string" and (
		mode == "stop" or mode == "pause" or mode == "start" or mode == "reset"
		or mode == "setEmissionRate"
	)) then error("Assertion failed: type(mode) == \"string\" and (\
...
\9)") end
	...
	if not (false) then error(string.format("particle system id '%s' not found", id)) end
end
