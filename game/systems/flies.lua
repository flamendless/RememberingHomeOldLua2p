...
function Flies:generate_flies(n, start_p, min_dist)
	if not (type(n) == "number" and n > 0) then error("Assertion failed: type(n) == \"number\" and n > 0") end
	if not (start_p:type() == "vec2") then error("Assertion failed: start_p:type() == \"vec2\"") end
	if not (type(min_dist) == "number" and min_dist > 0) then error("Assertion failed: type(min_dist) == \"number\" and min_dist > 0") end
...
function Flies:update(dt)
	for _, e in ipairs(self.pool) do
		local pos = e.pos
		local pv = e.pos.value
		local ref = e.ref_pos.value
