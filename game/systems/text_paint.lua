...
function TextPaint:show_text_paint(e, dur, widest)
	if not (e.__isEntity and e.text_with_paint) then error("Assertion failed: e.__isEntity and e.text_with_paint") end
	if not (type(dur) == "number") then error("Assertion failed: type(dur) == \"number\"") end
	if widest then if not (type(widest) == "string") then error("Assertion failed: type(widest) == \"string\"") end end
...
function TextPaint:fade_text_paint(e, dur, on_complete)
	...
	if not (e.text_with_paint) then error("Assertion failed: e.text_with_paint") end
	if not (e.text_with_paint.e_paint.paint) then error("Assertion failed: e.text_with_paint.e_paint.paint") end
	if not (type(dur) == "number") then error("Assertion failed: type(dur) == \"number\"") end
	if on_complete then if not (type(on_complete) == "function") then error("Assertion failed: type(on_complete) == \"function\"") end end
	local delay = 0
...
function TextPaint:show_paint(e, dur_in, widest)
	if not (e.__isEntity) then error("Assertion failed: e.__isEntity") end
	if not (type(dur_in) == "number") then error("Assertion failed: type(dur_in) == \"number\"") end
	if widest then if not (type(widest) == "string") then error("Assertion failed: type(widest) == \"string\"") end end
	if e.static_text then if not (e.static_text ~= nil) then error("Assertion failed: e.static_text ~= nil") end end
	if e.text then if not (e.text ~= nil) then error("Assertion failed: e.text ~= nil") end end
	local str
	local text = e.text
	...
	local paint = Concord.entity(self.world)
		...
		:give("transform", 0, vec2(sx, sy), vec2(0.5, 0.5))
		:give("color", {1, 1, 1, 0})
		:give("paint")
		:give("ui_element")
		:ensure("key")
...
function TextPaint:fade_paint(paint, dur, delay)
	if not (paint.__isEntity) then error("Assertion failed: paint.__isEntity") end
	if not (type(dur) == "number") then error("Assertion failed: type(dur) == \"number\"") end
	if not (paint.paint) then error("Assertion failed: paint.paint") end
	if delay then if not (type(delay) == "number") then error("Assertion failed: type(delay) == \"number\"") end end
