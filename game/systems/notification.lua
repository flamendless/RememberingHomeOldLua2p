...
function Notification:speech_bubble_update(_, e)
	...
	if not (e.notification) then error("Assertion failed: e.notification") end
	if not (e.id.value == "speech_bubble") then error("got " .. e.id.value) end
...
function Notification:create_speech_bubble(e_player)
	if not (e_player.__isEntity and e_player.player) then error("Assertion failed: e_player.__isEntity and e_player.player") end
