...
function RandomizeUV:reset_uv()
	...
	for _, e in ipairs(self.pool_room_item) do
		local id = e.id.value
		if not (self.orig_data[id]) then error(id .. " is missing orig quad data") end
...
function RandomizeUV:multi_randomize_uv(dur, chance)
	if not (type(dur) == "number" and dur > 0) then error("Assertion failed: type(dur) == \"number\" and dur > 0") end
	if not (type(chance) == "number" and chance > 0) then error("Assertion failed: type(chance) == \"number\" and chance > 0") end
