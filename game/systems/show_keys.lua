...
local function Assemble(e, key)
	e:give("atlas", Atlases.Keys.frames[key])
...
function ShowKeys:create_key_with_text(id, txt, key)
	...
	if not (type(txt) == "string") then error("Assertion failed: type(txt) == \"string\"") end
	local ww, wh = love.graphics.getDimensions()
	self.keys[id] = Concord.entity(self.world):assemble(Assemble, key)
		:give("id", "key_" .. id)
		:give("pos", vec2(ww - 8, wh - 8))
		:give("transform", 0, vec2(1, 1), vec2(0.5, 1))
		:give("quad_transform", 0, vec2(3, 3), vec2(1, 1))
		:give("color", {1, 1, 1, 1})
	...
	self.texts[id] = Concord.entity(self.world)
		...
		:give("ui_element")
		:give("transform", 0, vec2(1, 1), vec2(1, 1))
...
function ShowKeys:show_key(id, bool)
	...
	if not (e ~= nil) then error("Assertion failed: e ~= nil") end
	if not bool then
		e:give("hidden")
	else
		e:remove("hidden")
