local function calc_e_controller_origin(e)
	ASSERT(e.__isEntity)
...
function Transform:init(world)
	...
	self.pool.onAdded = function(pool, e)
		...
		elseif text then
			...
			h = font:getHeight(" ")
		elseif s_text then
			if not (s_text) then error("renderer first before transform system") end
...
function Transform:update_anchor(e)
	...
	if not (e_target) then error("Assertion failed: e_target") end
	local target_pos = e_target.pos
	local target_sprite = e_target.sprite
	if not (target_sprite.image:type() == "Image") then error("Assertion failed: target_sprite.image:type() == \"Image\"") end
	...
	if anchor.pad_x then
		x = x + anchor.pad_x
	end
	if anchor.pad_y then
		y = y + anchor.pad_y
	end
