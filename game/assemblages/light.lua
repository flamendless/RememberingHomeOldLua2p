...
function Light.point(e, x, y, z, size, diffuse, dir)
	...
	if not (type(z) == "number") then error("Assertion failed: type(z) == \"number\"") end
...
function Light.spot(e, x, y, z, dir, size, diffuse)
	...
	if not (type(y) == "number") then error("Assertion failed: type(y) == \"number\"") end
	if not (type(z) == "number") then error("Assertion failed: type(z) == \"number\"") end
	if not (type(dir) == "table") then error("Assertion failed: type(dir) == \"table\"") end
	if not (type(size) == "number") then error("Assertion failed: type(size) == \"number\"") end
	if not (type(diffuse) == "table") then error("Assertion failed: type(diffuse) == \"table\"") end
...
function Light.fl_spot(e, e_player, sync_data)
	if not (e.__isEntity) then error("Assertion failed: e.__isEntity") end
	if not (e_player.__isEntity) then error("Assertion failed: e_player.__isEntity") end
	if not (type(sync_data) == "table") then error("Assertion failed: type(sync_data) == \"table\"") end
