...
function Player.base(e, x, y, speed_data, can)
	if can then if not (type(can) == "table") then error("Assertion failed: type(can) == \"table\"") end end
	if can.move then if not (type(can.move) == "boolean") then error("Assertion failed: type(can.move) == \"boolean\"") end end
