...
function Intro.parallax(e, tag, scale)
	...
	if scale then if not (type(scale) == "number") then error("Assertion failed: type(scale) == \"number\"") end end
	if not (speed[tag]) then error("Assertion failed: speed[tag]") end
	if not (z_index[tag]) then error("Assertion failed: z_index[tag]") end
