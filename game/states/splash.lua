...
function Splash:create_splash_wits()
	...
	self.splash_wits = Concord.entity(self.world):assemble(Assemblages.Common.animated_sprite,
...
function Splash:do_glitch(time, delay)
	if not (type(time) == "number") then error("Assertion failed: type(time) == \"number\"") end
