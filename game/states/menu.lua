...
local target_x = 176
local tools = {"Manjaro","i3-Gaps","Discord","Löve Framework","Luapreprocess","Vim","Aseprite","Audacity","Export-TextureAtlas","makelove","msdf-bmfont"}
local libs = {"Anim8","Batteries","beehive","Bitser","Bump-niji","Concord","Enum","Flux","Gamera","HUMP","JProf","Lily","Log","Lume","ngrading","Outliner","Peeker","ReflowPrint","SDF","Semver","Slab","Splashes","strict","TimelineEvents"}
...
function Menu:setup_about()
	...
	local about_links = {"https://twitter.com/@flam8studio","https://discord.gg/2W4tyyV","https://flamendless.itch.io","mailto:flamendless.studio@gmail.com"}
...
function Menu:hide_main_menu(duration)
	if duration then if not (type(duration) == "number") then error("Assertion failed: type(duration) == \"number\"") end end
	for _, e in ipairs(self.pool_main_menu) do
		e:give("target_color", Palette.get("white", 0), duration or duration_hide)
		:give("lerp_on_finish_multi",
...
function Menu:open_url(str)
	if not (type(str) == "string") then error("Assertion failed: type(str) == \"string\"") end
