...
function Glitch:send_values_to_shader(new_values)
	if not (type(new_values) == "table") then error("Assertion failed: type(new_values) == \"table\"") end
...
function Glitch:do_random_glitch(reset_after)
	if reset_after then if not (type(reset_after) == "number") then error("Assertion failed: type(reset_after) == \"number\"") end end
