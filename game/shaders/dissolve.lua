...
function Dissolve:new(texture, duration)
	if not (texture:type() == "Image") then error("Assertion failed: texture:type() == \"Image\"") end
	if not (type(duration) == "number") then error("Assertion failed: type(duration) == \"number\"") end
...
function Dissolve:set_kind(kind)
	if not (type(kind) == "number") then error("Assertion failed: type(kind) == \"number\"") end
	self.kind = kind
end
...
function Dissolve:draw(fn)
	if not (type(fn) == "function") then error("Assertion failed: type(fn) == \"function\"") end
...
function Dissolve:draw_emit(world, ev_name)
	...
	if not (type(ev_name) == "string" and ev_name ~= "") then error("Assertion failed: type(ev_name) == \"string\" and ev_name ~= \"\"") end
	if self.flag_process then
		love.graphics.setShader(self.shader_code)
		world:emit(ev_name)
		love.graphics.setShader()
