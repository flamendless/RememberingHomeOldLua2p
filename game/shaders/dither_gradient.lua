...
function DitherGradient:new(is_active, t_dither, t_pal, values)
	if is_active then if not (type(is_active) == "boolean") then error("Assertion failed: type(is_active) == \"boolean\"") end end
	if t_dither then if not (t_dither:type() == "Image") then error("Assertion failed: t_dither:type() == \"Image\"") end end
	if t_pal then if not (t_pal:type() == "Image") then error("Assertion failed: t_pal:type() == \"Image\"") end end
	if values then if not (type(values) == "table") then error("Assertion failed: type(values) == \"table\"") end end
	...
	if not (self.depth >= ranges.depth.min and self.depth <= ranges.depth.max) then error("Assertion failed: self.depth >= ranges.depth.min and self.depth <= ranges.depth.max") end
	if not (self.dither_size >= ranges.dither_size.min and self.dither_size <= ranges.dither_size.max) then error("Assertion failed: self.dither_size >= ranges.dither_size.min and self.dither_size <= ranges.dither_size.max") end
	if not (self.contrast >= ranges.contrast.min and self.contrast <= ranges.contrast.max) then error("Assertion failed: self.contrast >= ranges.contrast.min and self.contrast <= ranges.contrast.max") end
	if not (self.offset >= ranges.offset.min and self.offset <= ranges.offset.max) then error("Assertion failed: self.offset >= ranges.offset.min and self.offset <= ranges.offset.max") end
...
function DitherGradient:debug_slider(id, val, min, max, opt)
	...
	if Slab.InputNumberSlider(id, val, min, max, opt) then
		if not (self[id]) then error("Assertion failed: self[id]") end
