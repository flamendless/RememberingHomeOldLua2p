...
function MotionBlur:new(canvas, config)
	...
	if config then if not (type(config) == "table") then error("Assertion failed: type(config) == \"table\"") end end
...
function MotionBlur:set_angle(angle)
	if not (type(angle) == "number") then error("Assertion failed: type(angle) == \"number\"") end
	self.shader_code:send("u_angle", angle)
end

function MotionBlur:set_strength(strength)
	if not (type(strength) == "number") then error("Assertion failed: type(strength) == \"number\"") end
