...
local function set_node(e, tag)
	...
	if not (type(tag) == "string") then error("Assertion failed: type(tag) == \"string\"") end
...
local function is_current_node_bt(e, ...)
	if not (e.__isEntity and e.behavior_tree) then error("Assertion failed: e.__isEntity and e.behavior_tree") end
	local nodes = {...}
	if not (type(nodes) == "table" and #nodes > 0) then error("Assertion failed: type(nodes) == \"table\" and #nodes > 0") end
	return function()
...
local function is_current_anim(e, tag)
	if not (e.__isEntity and e.behavior_tree and e.animation) then error("Assertion failed: e.__isEntity and e.behavior_tree and e.animation") end
	if not (type(tag) == "string") then error("Assertion failed: type(tag) == \"string\"") end
...
local function is_current_anim_done(e, tag)
	...
	return (current_frame.value == current_frame.limit) and "success" or "running"
end
...
local function has_component_bt(e, component)
	if not (e.__isEntity and e.behavior_tree) then error("Assertion failed: e.__isEntity and e.behavior_tree") end
	if not (type(component) == "string") then error("Assertion failed: type(component) == \"string\"") end
	return function()
...
local function random(tbl)
	if not (type(tbl) == "table" and #tbl > 0) then error("Assertion failed: type(tbl) == \"table\" and #tbl > 0") end
	
	for _, bt in ipairs(tbl) do if not (type(bt) == "function") then error("Assertion failed: type(bt) == \"function\"") end end
...
local function wait_random_bt(e, min, max)
	if not (e.__isEntity and e.behavior_tree) then error("Assertion failed: e.__isEntity and e.behavior_tree") end
	if not (type(min) == "number") then error("Assertion failed: type(min) == \"number\"") end
	if not (type(max) == "number") then error("Assertion failed: type(max) == \"number\"") end
	if not (min > 0 and min < max) then error("Assertion failed: min > 0 and min < max") end
	local elapsed = 0
