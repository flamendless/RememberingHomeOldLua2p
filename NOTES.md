...
3. Generate via `./run.sh gen_atlas {new_room}` then copy with `./run.sh copy_res`
...
7. Add entry to the following (or run `./run.sh new_room PascalCaseName snake_case` to scaffold steps 4–7 and data stubs):
