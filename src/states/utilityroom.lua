...
function UtilityRoom:state_setup()
	...
	Concord.entity(self.world):assemble(Assemblages.Common.bg, "utility_room")

	self.world:emit("create_room_bounds", w, h)
	self.world:emit("parse_room_items", "utility_room")
	...
	self.world:emit("set_ambiance", Palette.get_diffuse("ambiance_utility_room"))
...
function UtilityRoom:ev_draw_ex()
