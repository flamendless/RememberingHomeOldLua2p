...
function AnimationOld:setup_animation(e, data, on_loop)
	...
	if not multi then
		...
		local cache = self.cache_multi_animation[e:get("id").value]
	...
	local current_frame = e:get("current_frame")
	...
	Log.info("setup animation done", "id", e:get("id").value, current_tag)
end
...
function AnimationOld:init(world)
	...
	self.pool.onAdded = function(pool, e)
		local data = e:get("animation_data_old")
		local animation = e:get("animation_old")
	...
	self.pool_multi.onAdded = function(pool, e)
		self.cache_multi_animation[e:get("id").value] = {}
		local multi = e:get("multi_animation_data_old")
		local animation = e:get("animation_old")
	...
	self.pool_pause.onAdded = function(pool, e)
		...
		local pause_at = e:get("animation_pause_at_old")
	...
	self.pool_change.onAdded = function(pool, e)
		local cat = e:get("change_animation_tag_old")
...
function AnimationOld:update(dt)
	for _, e in ipairs(self.pool) do
		...
		if animation.is_playing then
			local stop = e:get("animation_stop_old")
			if stop then
			...
			local on_update = e:get("animation_on_update")
			if on_update then
...
if DEV then
	...
	function Animation:debug_update(dt)
		...
		if selected_e then
			local multi = selected_e:get("multi_animation_data_old")
			if multi then
				local multi_anim_data = multi.data
				...
				if Slab.BeginComboBox("cb_anim", { Selected = selected_anim }) then
					for tag in pairs(multi_anim_data) do
						if Slab.TextSelectable(tag) then
							selected_anim = tag
							local cache = self.cache_multi_animation[selected_e:get("id").value]
							if cache then
								cache[tag] = nil
							end
				...
				local animation = selected_e:get("animation_old")
				if selected_anim then
					Slab.Text("Frame")
					Slab.SameLine()
					local cur_frame = selected_e:get("current_frame")
					local max = multi_anim_data[selected_anim].n_frames
					local anim_pause_at = selected_e:get("animation_pause_at_old")
					local anim_stop = selected_e:get("animation_stop_old")
					...
					local anim_loop = animation.stop_on_last
					local cat = selected_e:get("change_animation_tag_old")
